...
def ensure_news_table():
    ...
    lakebase.run_write(
        """
        CREATE INDEX IF NOT EXISTS idx_ticker_news_symbol 
        ON ticker_news(symbol)
        """
    )
    lakebase.run_write(
        """
        CREATE INDEX IF NOT EXISTS idx_ticker_news_published 
        ON ticker_news(published_at DESC)
        """
    )
...
def get_news(symbol: str):
    ...
    First checks the database for cached news (fetched within last 6 hours).
    If stale or missing, fetches from Massive API, stores in DB, and returns.
    """
    ensure_news_table()
    ...
    # Check for cached news (fetched within last 6 hours)
    cached_news = lakebase.run_query(
        """
        SELECT id, symbol, title, description, article_url, image_url, 
               publisher, published_at, fetched_at
        FROM ticker_news
        WHERE symbol = %s 
    ...
    if cached_news:
        logger.info(f"Returning {len(cached_news)} cached news articles for {symbol}")
        return jsonify({"symbol": symbol, "news": cached_news, "cached": True})
    
    # Otherwise, fetch from API
    client = MassiveClient()
    try:
        data = client.get_news(symbol, limit=10)
    except requests.HTTPError as e:
        logger.error(f"Failed to fetch news for {symbol}: {e}")
        return jsonify({"error": f"Could not fetch news for {symbol}"}), 400
    
    # Parse and store news articles
    ...
    for article in articles:
        ...
        publisher = publisher_data.get("name", "") if isinstance(publisher_data, dict) else str(publisher_data)
        ...
        try:
            ...
            lakebase.run_write(
                """
                INSERT INTO ticker_news (symbol, title, description, article_url, 
                                        image_url, publisher, published_at, fetched_at)
                VALUES (%s, %s, %s, %s, %s, %s, %s, NOW())
                ON CONFLICT DO NOTHING
                """,
                (symbol, title, description, article_url, image_url, publisher, published_utc),
            )
    ...
    logger.info(f"Fetched and stored {len(stored_articles)} news articles for {symbol}")
    return jsonify({"symbol": symbol, "news": stored_articles, "cached": False})
